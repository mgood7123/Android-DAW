...
namespace XUI.Graphics {
    ...
    public readonly struct Color4 : IEquatable<Color4>, IFormattable
    {
        ...
        {
            return new(
                Maths.Lerp(start.R, end.R, amount),
                Maths.Lerp(start.G, end.G, amount),
                Maths.Lerp(start.B, end.B, amount),
                Maths.Lerp(start.A, end.A, amount)
                );
        }
        ...
        {
            amount = Maths.SmoothStep(amount);
        ...
        {
            return new(
                Maths.SRgbToLinear(R),
                Maths.SRgbToLinear(G),
                Maths.SRgbToLinear(B),
                A);
        }
        ...
        {
            return new(
                Maths.LinearToSRgb(R),
                Maths.LinearToSRgb(G),
                Maths.LinearToSRgb(B),
                A);
        }
        ...
        private float Hue(float min, float max)
        {
            ...
            if (chroma <= Maths.ZeroTolerance)
                return 0.0f;

            // Calculate and return hue
            if (Maths.CompareEqual(G, max))
                return (B + 2.0f * chroma - R) / (6.0f * chroma);
            else if (Maths.CompareEqual(B, max))
        ...
        {
            // Avoid div-by-zero: result undefined
            if (max <= Maths.ZeroTolerance || min >= 1.0f - Maths.ZeroTolerance)
                return 0.0f;
        ...
        /// </returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static bool operator !=(Color4 left, Color4 right) => MathsVec128.CompareNotEqualAny(left._value, right._value);
        ...
        {
            string separator = System.Globalization.NumberFormatInfo.GetInstance(formatProvider).NumberGroupSeparator;

            return new System.Text.StringBuilder(9 + separator.Length * 3)
                .Append('<')
        ...
        {
            System.Diagnostics.Debug.Assert(index is >= 0 and < Count);
